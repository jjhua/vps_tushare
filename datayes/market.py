...
class Market():
    ...
    def TickRTSnapshot(self, securityID='', field=''):
        """
            ...
            证券可以是股票，指数， 部分债券或 基金。
        """
        code, result = self.client.getData(vs.TICKRTSNAPSHOT % (securityID,
                                                                field))
    ...
    def TickRTIntraDay(self, securityID='', endTime='', startTime='',
                       field=''):
        """
    ...
    def FutureTickRTIntraDay(self,
        ...
        """
        code, result = self.client.getData(vs.FUTURETICKRTINTRADAY %
                                           (instrumentID, endTime, startTime,
                                            field))
        return _ret_data(code, result)

    def FutureBarsOneDay(self, instrumentID='', date='', field=''):
        code, result = self.client.getData(vs.FUTUREBARINDAY % (instrumentID,
                                                                date, field))
        return _ret_data(code, result)
    ...
    def StockFactorsDateRange(self,
        ...
        """
        code, result = self.client.getData(vs.STOCKFACTORSDATERANGE %
                                           (secID, ticker, beginDate, endDate,
                                            field))
        return _ret_data(code, result)
    ...
    def BarRTIntraDayOneMinute(self, time='', field=''):
        """
            获取所有股票某一分钟的分钟线
        """
        code, result = self.client.getData(vs.BARRTINTRADAYONEMINUTE % (time,
                                                                        field))
        return _ret_data(code, result)
    ...
    def EquRTRank(self, desc='', exchangeCD='', field=''):
        """
            获取沪深股票涨跌幅排行
        """
        code, result = self.client.getData(vs.EQURTRANK % (desc, exchangeCD,
                                                           field))
        return _ret_data(code, result)
    ...
    def OptionTickRTSnapshot(self, optionId='', field=''):
        """
            高频数据，获取期权最新市场信息快照
        """
        code, result = self.client.getData(vs.OPTIONTICKRTSNAPSHOT % (optionId,
                                                                      field))
        return _ret_data(code, result)
    ...
    def MktHKEqud(self,
        ...
        """
        code, result = self.client.getData(vs.MKTHKEQUD %
    ...
    def MktBondd(self,
        ...
        """
        code, result = self.client.getData(vs.MKTBONDD %
                                           (secID, ticker, tradeDate,
                                            beginDate, endDate, field))
        return _ret_data(code, result)
    ...
    def MktRepod(self,
        ...
        """
        code, result = self.client.getData(vs.MKTREPOD %
                                           (secID, ticker, tradeDate,
                                            beginDate, endDate, field))
        return _ret_data(code, result)
    ...
    def MktFundd(self,
        ...
        """
        code, result = self.client.getData(vs.MKTFUNDD %
                                           (secID, ticker, tradeDate,
                                            beginDate, endDate, field))
        return _ret_data(code, result)
    ...
    def MktIdxd(self,
        ...
        """
        code, result = self.client.getData(vs.MKTIDXD %
                                           (indexID, ticker, tradeDate,
                                            beginDate, endDate, field))
        return _ret_data(code, result)
    ...
    def MktBlockd(self,
        ...
        """
        code, result = self.client.getData(
            vs.MKTBLOCKD % (secID, ticker, tradeDate, assetClass, beginDate,
                            endDate, field))
        return _ret_data(code, result)
    ...
    def MktOptd(self,
        ...
        """
        code, result = self.client.getData(vs.MKTOPTD %
                                           (optID, secID, ticker, tradeDate,
                                            beginDate, endDate, field))
        return _ret_data(code, result)
    ...
    def MktEqudAdj(self,
        ...
        """
        code, result = self.client.getData(vs.MKTEQUDADJ %
                                           (secID, ticker, tradeDate,
                                            beginDate, endDate, field))
        return _ret_data(code, result)
    ...
    def MktLimit(self, secID='', ticker='', tradeDate='', field=''):
        ...
        """
        code, result = self.client.getData(vs.MKTLIMIT % (secID, ticker,
                                                          tradeDate, field))
        return _ret_data(code, result)
