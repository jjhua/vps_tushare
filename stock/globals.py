...
def global_realtime(symbols=None):
    ...
    request = Request(
        ct.LIVE_DATA_URL % (ct.P_TYPE['http'], ct.DOMAINS['sinahq'],
    ...
    for cont in content:
        ...
        if (symbol == 'sh000001') or (symbol == 'sz399001'):
            ...
            arrs = [
                symbol, valarr[0], valarr[3], price - preclose, chg,
                valarr[30] + ' ' + valarr[31]
            ]
        elif symbol == 'hkHSI':
            arrs = [
                symbol, valarr[1], valarr[6], valarr[7], valarr[8],
                valarr[17].replace('/', '-') + ' ' + valarr[18] + ':00'
            ]
        else:
            arrs = [
                symbolstr[3], valarr[0], valarr[1], valarr[2], valarr[3],
