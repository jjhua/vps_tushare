...
def profit_data(year=2017, top=25, retry_count=3, pause=0.001):
    ...
    if top == 'all':
        ...
        if isinstance(top, int):
            ...
            for idx in range(1, int(pages)):
                df = df.append(
                    _dist_cotent(year, idx, retry_count, pause),
                    ignore_index=True)
            return df.head(top)
        else:
...
def _dist_cotent(year, pageNo, retry_count, pause):
    for _ in range(retry_count):
        ...
        try:
            if pageNo > 0:
                ct._write_console()
            html = lxml.html.parse(rv.DP_163_URL %
                                   (ct.P_TYPE['http'], ct.DOMAINS['163'],
...
def _get_forecast_data(year, quarter, pageNo, dataArr):
    ...
    try:
        ...
        html = lxml.html.parse(
            ct.FORECAST_URL %
            (ct.P_TYPE['http'], ct.DOMAINS['vsf'], ct.PAGES['fd'], year,
...
def xsg_data(year=None, month=None, retry_count=3, pause=0.001):
    ...
    for _ in range(retry_count):
        time.sleep(pause)
        try:
            request = Request(rv.XSG_URL %
                              (ct.P_TYPE['http'], ct.DOMAINS['em'],
...
def _holding_cotent(start, end, pageNo, retry_count, pause):
    for _ in range(retry_count):
        ...
        if pageNo > 0:
            ct._write_console()
        try:
            request = Request(rv.FUND_HOLDS_URL %
                              (ct.P_TYPE['http'], ct.DOMAINS['163'],
...
def _newstocks(data, pageNo, retry_count, pause):
    for _ in range(retry_count):
        ...
        ct._write_console()
        try:
            url = rv.NEW_STOCKS_URL % (ct.P_TYPE['http'], ct.DOMAINS['vsf'],
...
def sh_margins(start=None, end=None, retry_count=3, pause=0.001):
    ...
    ct._write_head()
    df = _sh_hz(
        data, start=start, end=end, retry_count=retry_count, pause=pause)
    return df
...
def _sh_hz(data,
    ...
    for _ in range(retry_count):
        ...
        try:
            ...
            beginPage = pageNo
            endPage = pageNo + 4
            url = rv.MAR_SH_HZ_URL % (ct.P_TYPE['http'],
                                      ct.DOMAINS['sseq'], ct.PAGES['qmd'],
                                      _random(5), start, end, tail, _random())
            ...
            datapage = int(pagecount / 5 + 1
                           if pagecount % 5 > 0 else pagecount / 5)
...
def _sh_mx(data,
    ...
    for _ in range(retry_count):
        ...
        try:
            ...
            ref = rv.MAR_SH_HZ_REF_URL % (ct.P_TYPE['http'], ct.DOMAINS['sse'])
            clt = Client(
                rv.MAR_SH_MX_URL % (ct.P_TYPE['http'],
                                    ct.DOMAINS['sseq'], ct.PAGES['qmd'],
                                    _random(5), date, symbol, start, end, tail,
                                    _random()),
                ref=ref,
...
def _sz_hz(date='', retry_count=3, pause=0.001):
    for _ in range(retry_count):
        ...
        ct._write_console()
        try:
            request = Request(rv.MAR_SZ_HZ_URL %
...
def sz_margin_details(date='', retry_count=3, pause=0.001):
    ...
    for _ in range(retry_count):
        time.sleep(pause)
        try:
            request = Request(rv.MAR_SZ_MX_URL %
                              (ct.P_TYPE['http'], ct.DOMAINS['szse'],
                               ct.PAGES['szsefc'], date))
            lines = urlopen(request, timeout=10).read()
            if len(lines) <= 200:
...
def top10_holders(code=None,
    ...
    for _ in range(retry_count):
        time.sleep(pause)
        try:
            request = Request(rv.TOP10_HOLDERS_URL % (ct.P_TYPE['http'],
                                                      ct.DOMAINS['gw'], gdtype,
                                                      code.upper()))
            lines = urlopen(request, timeout=10).read()
